...
import { Account, Category, Transaction, Budget, UserStreak } from '../types';
...
export const getSeedData = () => {
  ...
  addTx(1, 'expense', 12.50, 'Chipotle', 'cat_1', 'acc_2'); // Yesterday
  ...
  // --- Simulate Streak ---
  // We assume the user has been active recently, last log being yesterday (1 day ago)
  // This allows the user to log today and see the streak animation.
  const streak: UserStreak = {
      currentCount: 12,
      longestCount: 24,
      lastLogDate: format(subDays(now, 1), 'yyyy-MM-dd')
  };

  return { accounts, transactions, budgets, categories, streak };
};
